...
#ifdef _DEBUG
#define DebugMsg(p_message,...) if(g_session->GetSocketLogging() == SOCK_LOGGING_FULLTRACE)\
                                   g_session->GetLogfile()->AnalysisLog(_T(__FUNCTION__),LogType::LOG_INFO, true,p_message,__VA_ARGS__)
#define LogError(p_message,...) if(g_session->GetSocketLogging() == SOCK_LOGGING_FULLTRACE)\

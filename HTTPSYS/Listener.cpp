...
Listener::Initialize(int p_tcpListenPort)
{
  ...
  for (i = 0, AI = AddrInfo; AI != nullptr; AI = AI->ai_next)
  {
    ...
    m_listenSockets[i] = WSASocketW(AI->ai_family,SOCK_STREAM,IPPROTO_TCP,NULL,0,WSA_FLAG_OVERLAPPED | WSA_FLAG_NO_HANDLE_INHERIT);
    if(m_listenSockets[i] == INVALID_SOCKET)
    {
...
{
  m_listenerThread = (HANDLE)_beginthreadex(nullptr,0,ListenerWorker,this,0,nullptr);
}
...
void Listener::StopListener(void)
{
  ...
  if (m_listenerThread)
  {
    WaitForSingleObject(m_listenerThread,INFINITE); // Will auto delete
  }
  m_listenerThread = NULL;
}
